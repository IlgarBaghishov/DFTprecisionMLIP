...
subsample_size = int(sys.argv[1])  # Number of configurations to subsample
...
for i in range(3):
    ...
    for j in range(n_repetitions):
        ...
        coeffs, *_ = lstsq(aw[hlfpnt1_sub], bw_train[hlfpnt1_sub], 1.0e-13)
        ...
        results.append([
            ...
            np.sqrt(np.mean(np.square(residual[hlfpnt1_sub][energy_mask[hlfpnt1_sub]])))/eweight,
            np.sqrt(np.mean(np.square(residual[hlfpnt1_sub][force_mask[hlfpnt1_sub]]))),
            np.sqrt(np.mean(np.square(residual[hlfpnt1][energy_mask[hlfpnt1]])))/eweight,
            np.sqrt(np.mean(np.square(residual[hlfpnt1][force_mask[hlfpnt1]]))),
            np.sqrt(np.mean(np.square(residual[hlfpnt2][energy_mask[hlfpnt2]])))/eweight,
            np.sqrt(np.mean(np.square(residual[hlfpnt2][force_mask[hlfpnt2]])))
        ])
