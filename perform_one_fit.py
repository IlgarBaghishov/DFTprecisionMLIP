...
bw_train = np.load(os.path.join(data_dir, 'bw_' + str(precision_to_train_on) + '.npy'))
bw_errors = np.load(os.path.join(data_dir, 'bw_' + str(precision_to_calc_errors_on) + '.npy'))
...
print("Energy training RMSE is", np.sqrt(np.mean(np.square(residual[train_idxs][energy_mask[train_idxs]]))))
print("Force training RMSE is", np.sqrt(np.mean(np.square(residual[train_idxs][force_mask[train_idxs]]))))
print("Energy testing RMSE is", np.sqrt(np.mean(np.square(residual[test_idxs][energy_mask[test_idxs]]))))
print("Force testing RMSE is", np.sqrt(np.mean(np.square(residual[test_idxs][force_mask[test_idxs]]))))
